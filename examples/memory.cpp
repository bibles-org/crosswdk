...
export namespace memory {
    arch::address translation_example(const arch::address pml4, const arch::address linear_address) {
        ...
        const auto pdpe_large = static_cast<arch::pdpe_1gb*>(
                static_cast<arch::address>(static_cast<std::uint64_t>(pml4e.page_frame_number())) << 12
        ...
        if (pdpe_large.page_size() == true) {
            return (static_cast<arch::address>(static_cast<std::uint64_t>(pdpe_large.page_frame_number())) << 30) +
                   linear_address.offset_1gb();
        }

        const arch::pdpe pdpe{pdpe_large};
        const auto pde_large = static_cast<arch::pde_2mb*>(
                static_cast<arch::address>(static_cast<std::uint64_t>(pdpe.page_frame_number())) << 12
        ...
        if (pde_large.page_size() == true) {
            return (static_cast<arch::address>(static_cast<std::uint64_t>(pde_large.page_frame_number())) << 21) +
                   linear_address.offset_2mb();
        }

        const arch::pde pde{pde_large};
        const auto pte = static_cast<arch::pte*>(
                static_cast<arch::address>(static_cast<std::uint64_t>(pde.page_frame_number())) << 12
        ...
        // 4kb mapping
        return (static_cast<arch::address>(static_cast<std::uint64_t>(pte.page_frame_number())) << 12) +
